#!/usr/bin/env python3
"""VulnForge — launcher único (Missão 1: "unzip e um comando").
...
precisa conhecer. Num clone limpo, sem nada instalado, ele:
  ...
  3. instala o VulnForge e dependências (``.[pdf,tui]`` por padrão; ``.[ai]`` /
     ``.[dev]`` sob demanda);
...
bootstrap. Argumentos são repassados à CLI:
    ...
    python3 vulnforge.py                 # menu interativo (Novo Scan = wizard)
    python3 vulnforge.py scan alvo.com   # scan direto (headless/CI)
    python3 vulnforge.py doctor          # diagnóstico  (--install provisiona ferramentas)
...
def _check_python() -> bool:
    ...
    print(f"VulnForge requer Python 3.11+ — você tem {have}.")
    print(f"Baixe/atualize: {_python_download_url()}")
    print("Depois rode de novo: python3 vulnforge.py")
    return False
...
def _config_dir() -> Path:
    base = os.environ.get("VULNFORGE_CONFIG_DIR") or os.path.join(
        os.path.expanduser("~"), ".config", "vulnforge"
    )
    return Path(base)
...
def _venv_has_package(py: Path) -> bool:
    ...
    result = subprocess.run([str(py), "-I", "-c", "import vulnforge.cli.main"], capture_output=True)
...
def _pip_install_current(opts: dict[str, bool]) -> bool:
    ...
    print("· Instalando o VulnForge no ambiente atual (--no-venv)…")
    for candidate in (_install_spec(opts), "."):
...
def _parse_launcher_flags(argv: list[str]) -> tuple[dict[str, bool], list[str]]:
    ...
    for arg in argv:
        if arg in _LAUNCHER_FLAGS:
            ...
            # `vulnforge.py doctor --help` etc. seguem para a CLI.
            opts["help"] = True
        else:
            cli_args.append(arg)
...
def _print_launcher_help() -> None:
    ...
    print("  python3 vulnforge.py                 # menu interativo (Novo Scan = wizard)")
    print("  python3 vulnforge.py scan ALVO       # scan direto (headless/CI)")
    print("  python3 vulnforge.py doctor          # diagnóstico (--install provisiona ferramentas)")
    print("  python3 vulnforge.py serve           # dashboard web")
    print("  python3 vulnforge.py <cmd> --help    # ajuda detalhada de um comando da CLI")
