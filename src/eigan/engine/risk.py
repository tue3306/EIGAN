...
consultado (:mod:`eigan.engine.feeds`); do contrário saem ``UNVERIFIED`` e o
...
o ``rationale`` de cada :class:`~eigan.findings.schema.RiskScore` explica em
...
class RiskScorer:
    ...
    def _score_one(self, f: Finding) -> RiskScore:
        ...
        if self._feeds is not None and self._feeds.kev_available:
            ...
            provenance["kev"] = "UNVERIFIED (rode `eigan feeds update`)"
