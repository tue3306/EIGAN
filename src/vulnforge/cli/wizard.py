"""Wizard interativo — ``vulnforge`` sem argumentos (§F).
...
:mod:`~vulnforge.cli.session`) → execução com progresso → oferta de PDF.
"""
...
def run_wizard(db: str = "vulnforge.db") -> int:
    click.secho("┌─ Assistente VulnForge ──────────────────────────────────┐", fg="cyan")
