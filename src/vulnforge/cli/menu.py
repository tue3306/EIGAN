"""Menu de produto — porta de entrada interativa do VulnForge (Missão 0).

`vulnforge` sem argumentos (ou `python3 vulnforge.py`) abre este **menu
...
injetáveis, como em :mod:`~vulnforge.cli.session`).
...
def action_doctor(
    *, db: str, input_fn: Callable[[str], str] = input, echo: Callable = print
    ...
    """Diagnóstico do ambiente (reaproveita `vulnforge doctor`)."""
...
def serve_app(
    *,
    ...
    echo(f"VulnForge — dashboard: {url}   ·   API: {url}/api/v1   ·   docs: {url}/docs")
    if open_browser:
    ...
    try:
        ...
        return
    try:
        uvicorn.run("vulnforge.api.app:app", host=host, port=port, log_level="warning")
...
def run_menu(
    *, db: str = "vulnforge.db", input_fn: Callable[[str], str] = input, echo: Callable = print
) -> int:
...
def run_frontend(db: str = "vulnforge.db") -> int:
