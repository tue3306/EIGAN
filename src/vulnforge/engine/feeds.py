...
Cache fora do repositório (``$VULNFORGE_CACHE_DIR`` ou ``~/.cache/vulnforge``).
...
log = logging.getLogger("vulnforge.feeds")
...
def default_cache_dir() -> Path:
    env = os.getenv("VULNFORGE_CACHE_DIR")
    base = Path(env) if env else Path.home() / ".cache" / "vulnforge"
    return base / "feeds"
...
def _http_get(url: str, timeout: int = 30) -> bytes:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "VulnForge/feeds"})
