...
Duas peças complementam o :mod:`~vulnforge.security.consent` (consent inline por
scan) e o :mod:`~vulnforge.security.scope` (trava dura por arquivo):

1. **Aceite de termo na 1ª execução**, gravado **fora do repositório**
   (``~/.config/vulnforge``), para não pedir de novo a cada scan.
...
def config_dir() -> Path:
    env = os.getenv("VULNFORGE_CONFIG_DIR")
    return Path(env) if env else Path.home() / ".config" / "vulnforge"
