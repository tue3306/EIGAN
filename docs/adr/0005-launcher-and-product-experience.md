...
entrada só via `pip install` + comando `vulnforge`, ou `python -m vulnforge`).
...
### 1. Launcher único `vulnforge.py` na raiz (só stdlib)
...
são repassados à CLI (`python3 vulnforge.py scan alvo.com`). Um atalho de shell
`./vulnforge` chama o mesmo script.

Detalhes que valem registro:

- **Colisão de nome resolvida.** Um arquivo `vulnforge.py` na raiz *sombreia* o
  pacote `vulnforge`. O launcher remove o diretório do script de `sys.path`
  (`_deshadow`) antes de qualquer import e detecta o pacote pelo **submódulo**
  `vulnforge.cli.main` (o script não é um pacote). A sondagem do venv usa
  `python -I` (isolado de cwd/PYTHON*), evitando o mesmo sombreamento.
- **Sem loop de bootstrap.** Uma flag de ambiente (`VULNFORGE_BOOTSTRAPPED`)
...
`vulnforge` sem argumentos passa a abrir um **menu numerado** (Novo Scan,
...
§13/§18 descrevem `vulnforge` sem argumentos abrindo **o wizard**. Passamos a
...
- **Renomear o pacote ou o script para evitar a colisão de nome.** Rejeitado: o
  usuário pediu explicitamente `python3 vulnforge.py`; a colisão é contornável
...
- ✅ `git clone && python3 vulnforge.py` funciona sem pré-requisitos além de
...
- ⚠️ Ter um `vulnforge.py` na raiz com o mesmo nome do pacote exige o cuidado de
  `sys.path` acima; rodar `import vulnforge` a partir da raiz do repo (sem o
  launcher) pega o script. Não é um fluxo suportado — usa-se `python3
  vulnforge.py` ou o comando instalado.
